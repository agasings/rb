...
function getAttachFile($R,$mod,$featured_img_uid,$editor)
{
	global $g,$r;
      ...
      $html='';
	$html.='
	 <li class="col-xs-3 px-0 dd-handle" data-id="'.$R['uid'].'" style="background-color: transparent">';

	 if($R['type']==2){
	      $html.='
            <div class="position-relative embed-responsive embed-responsive-1by1 bg-faded">
                <img class="img-fluid" src="'.$thumb_list.'">
                <div class="item-label">';
                    ...
                    <small class="d-none text-muted">'.getSizeFormat($R['size'],2).'</small>
								...
								if($mod=='upload')  $html.='<input type="hidden" name="attachfiles[]" value="['.$R['uid'].']"/>';
											$html.='
											<div class="position-absolute" style="top:-1px;left:-1px"><button type="button" class="btn btn-secondary btn-sm border-0 rounded-0" data-attach-act="delete" data-id="'.$R['uid'].'" data-role="attachList-menu-delete-'.$R['uid'].'" data-featured="" data-type="'.$type.'"><i class="fa fa-times" aria-hidden="true"></i></button></div>';
