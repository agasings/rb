...
if ($uid) {
  ...
  $QVAL1 .="d_modify='$d_modify',upload='$upload',log='$log',featured_img='$featured_img',linkedmenu='$linkedmenu',linkedshop='$linkedshop'";
  ...
  $_orign_category_members = getDbArray($table[$m.'index'],'data='.$R['uid'],'*','data','asc',0,1);
	...
	while($_ocm=db_fetch_array($_orign_category_members)) {
  	if(!strstr($category_members,'['.$_ocm['category'].']')) {
  		getDbDelete($table[$m.'index'],'category='.$_ocm['category'].' and data='.$uid);
    ...
    $mingid = getDbCnt($table[$m.'index'],'min(gid)','');
  ...
  $QKEY1.= "hit,comment,oneline,d_regis,d_modify,d_comment,upload,log,display,d_display,featured_img,format,linkedmenu,linkedshop";
  $QVAL1 = "'$s','$gid','$mbruid','$cid','$subject','$review','$content','$tag','$html',";
  $QVAL1.= "'0','0','0','$d_regis','','','$upfiles','$log','$display','$d_display','$featured_img','$format','$linkedmenu','$linkedshop'";
  getDbInsert($table[$m.'data'],$QKEY1,$QVAL1);

  $LASTUID = getDbCnt($table[$m.'data'],'max(uid)','');
  $QKEY2 = "mbruid,gid,data,type,d_regis";
  $QVAL2 = "'$mbruid','$gid','$LASTUID','$type','$d_regis'";
  ...
  $_category_members = array();
    ...
    {
      $_mingid = getDbCnt($table[$m.'index'],'min(gid)','');
      $_gid = $mingid ? $mingid-1 : 100000000;
      getDbInsert($table[$m.'index'],'site,data,category,depth,gid',"'".$s."','".$LASTUID."','".$_ct1."','".$_ct1_depth."','".$_gid."'");
      getDbUpdate($table[$m.'category'],'num=num+1','uid='.$_ct1);
    }
	}
...
if ($uid) {
  ...
  echo 'parent.$.notify({message: "포스트가 수정 되었습니다."},{type: "success"});';
  echo 'parent.$(".js-submit").prop("disabled",false);';
  echo '</script>';
  exit;
} else {
  setrawcookie('result_post_main', rawurlencode('포스트가 등록 되었습니다.'));
